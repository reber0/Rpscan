...
def main():
    ...
    if args.get("scantype") == "masscan":
        ps = PortScan(ip_list=live_host, all_ports=args.get("is_all_ports"), rate=args.get("rate"))
    ...
    if args.get("service") and len(port_open_dict)>0:
        ...
        ngps = NmapGetPortService(ip_port_dict=port_open_dict, thread_num=args.get("thread"))
